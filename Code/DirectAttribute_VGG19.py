...
class DirectAttribute_VGG19(object):
    ...
    def __init__(self, learning_rate, attribute_length, num_classes_all, attri_list_all, batch_size,
        ...
        self.predic_attr_ = dense(tf.reshape(self.pool5, [-1, 25088]),
        ...
        self.loss = (1.0/(1.0+self.regul)) * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=self.img_attribute, logits=self.predic_attr_)) +\
                    (self.regul / (1.0 + self.regul)) * tf.reduce_mean((self.img_label_all-self.predic_label)**2)
    ...
    def Get_label(self, attribute, label_attri):
        ...
        label_idx = tf.argmax(tf.matmul(attribute_input, tf.convert_to_tensor(label_attri.T)), axis=1)
