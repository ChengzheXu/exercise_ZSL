...
class DirectAttrbute_VGG19(object):
    """
    A direct attribute learner, use VGG19.
    """
    def __init__(self, learning_rate, vgg19_npy_path=None, trainable=False):
        """
        :param vgg19_npy_path: The parameter path.
        :param trainable: A bool value.
        ...
        self.img_tensor = tf.placeholder(tf.float32, shape=[None, img_height, img_width, 3])
        ...
        #                                              attribute_length), axis=1,
        #                                         training=True)), keep_prob=1-self.dropout)

        self.prediction = sigmoid(BatchNormalization(dense(tf.reshape(self.pool5, [-1, 25088]),
                                                      attribute_length), axis=1,
                                                training=True))

        self.loss = tf.nn.sigmoid_cross_entropy_with_logits\
            (labels=self.img_attribute, logits=self.prediction)
    ...
    def build(self, input_image, include_top=False,  train_mode=None):
        ...
        assert red.get_shape().as_list()[1:] == [img_width, img_height, 1]
        assert green.get_shape().as_list()[1:] == [img_width, img_height, 1]
        assert blue.get_shape().as_list()[1:] == [img_width, img_height, 1]
...
img_tensor = tf.convert_to_tensor(np.ones([batch_size, img_width, img_height, img_path]))
img_attribute = tf.convert_to_tensor(np.ones([batch_size, attribute_length]))

model = DirectAttrbute_VGG19((1e-3), vgg19_npy_path="./vgg19.npy")
init_op = tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init_op)
    [img_tensor, img_attribute, dropout_rate] = sess.run([img_tensor, img_attribute, tf.constant(dropout_rate)])
    print("Train the fully connected layers.")
    sess.run(model.train_op, feed_dict={model.img_tensor: img_tensor,
                                        model.img_attribute: img_attribute,
