...
class IndirectAttribute_VGG19(object):
    """
    A direct attribute learner, use VGG19.
    """
    def __init__(self, learning_rate, attribute_length, num_classes_all, num_classes_train, attri_list_all, attri_list_train,
                 vgg19_npy_path=None,
                 trainable=False,
                 img_shape=224,
                 regul=0.0):
        ...
        # h_fc1 = dropout(relu(BatchNormalization(dense(tf.reshape(self.pool5, [-1, 25088]),
        #                                              self.attribute_length), axis=1,
        #                                         training=True)), keep_prob=1-self.dropout)

        self.predic_label_train_ = dense(tf.reshape(self.pool5, [-1, 25088]),
                                         self.num_classes_train)

        self.predic_label_train = softmax(self.predic_label_train_)

        self.predic_attr = self.Get_attr(self.predic_label_train, self.attri_list_train)
        ...
        # 仅在train时是有意义的。
        self.loss = (1.0/(1.0+self.regul)) * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            labels=self.img_label_train, logits=self.predic_label_train_)) + \
                    (self.regul / (1.0 + self.regul)) * tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.img_attribute, logits=self.predic_attr))
    ...
    def build(self, input_image, include_top=False,  train_mode=None):
        ...
        if include_top:
            ...
            self.fc8 = self.fc_layer(self.relu7, 4096, self.num_classes_train, "fc8")
    ...
    def Get_label(self, attribute, label_attri_all):
        """
        Get the labels according to attributes.
        :param attribute: The attributes, shape = [batch_size, attribute_length].
        :param label_attri_all: The list of attributes for all classes (np.array), shape = [num_classes, attribute_length].
        Caution: the attributes must be in the order according to the labels.
        :return: The relative labels (one-hot), shape = [batch_size, num_classes_all].
        """
        with tf.Session() as sess:
            attribute = sess.run(attribute)
        label_idx = list(np.argmax(np.dot(attribute, label_attri_all.T), axis=1))

        label = np.zeros([np.shape(attribute)[0], np.shape(label_attri_all)[0]])
    ...
    def Get_attr(self, label_train, attri_list_train):
        """
        ...
        :param attri_list_train: The list of attributes (np.array), shape = [num_classes, attribute_length].
        Caution: the attributes must be in the order according to the labels.
        ...
        return tf.convert_to_tensor(np.dot(label_train, attri_list_train))
